...
export const NewHabitModal = () => {
  ...
  const { mutateAsync } = useMutation(
    async (data: { title: string; weekDays: number[] }) => {
  ...
  return (
    <form onSubmit={createNewHabit} className="w-full flex flex-col mt-6">
      <label htmlFor="title" className="font-semibold leading-tight"></label>
      <input
        type="text"
        id="title"
        value={title}
      ...
      <button
        type="submit"
        className="mt-6 p-4 rounded-lg flex items-center justify-center gap-3 font-semibold bg-green-600 hover:bg-green-500 transition-colors focus:ring-2 focus:ring-green-600 focus:ring-offset-2 focus:ring-offset-zinc-900"
      >
